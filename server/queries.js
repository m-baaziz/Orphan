...
let client,
  disordersCollection,
  phenotypesCollection,
  disordersClassificationCollection,
  phenotypesClassificationCollection;

async function initDb() {
  try {
    console.log('initalizing db connection ...');
    const client = await MongoClient.connect(`mongodb://${DB_HOST}:${DB_PORT}`, {
    ...
    disordersClassificationCollection = client.db(DB_NAME).collection(DISORDERS_CLASSIFICATION_COLLECTION);
    phenotypesClassificationCollection = client.db(DB_NAME).collection(PHENOTYPES_CLASSIFICATION_COLLECTION);
...
function filterPhenotypes(parentHPOId, text) {
  return phenotypesCollection.find({
    parents: parentHPOId,
    $or: [
      { name: { $regex: `.*${text}.*` } },
    ...
    await async.eachSeries(phenotypes, async(phenotype) => {
      try {
        ...
        disorders.forEach(({ orphaNumber }) => {
          disordersMap[orphaNumber] = disordersMap[orphaNumber] ? [...disordersMap[orphaNumber], phenotypeWithComment(phenotype)] : [];
        });
      } catch(e) {
        return Promise.reject(e);
      }
...
function interStatementDisorderScoreUpdate(lastScore, keyPhenotypes) {
  const incomingScore = _.sum(keyPhenotypes.map(keyPhenotype => computePhenotypeScore(keyPhenotype, keyPhenotype.comment)));
...
async function main() {
  ...
  try {
    ...
    const sortedDisorders = _.sortBy(Object.keys(disordersScores), [orphaNumber => -disordersScores[orphaNumber]]);
